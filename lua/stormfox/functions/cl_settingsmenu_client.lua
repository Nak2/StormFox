	...
	-- Settings
		...
		-- SF Light
			...
			local label = mgui.Create("DLabel",panel)
				...
				label:SizeToContentsX(5)
				label:SetPos(20,10 + element_size * 15)
			local convar = GetConVar("sf_dynamiclightamount")
			local wr = mgui.Create("Slider",panel)
					wr:SetPos(20 + label:GetWide(),10 + element_size * 15)
					wr:SetSize(200,20)
					wr:SetMax(5)
					wr:SetMin(1)
					wr:SetValue(convar:GetInt())
			...
			--sf_allow_sunbeams
				...
				p:SetPos(20,10 + element_size * 16)
				label:SetPos(30 + p:GetWide(),10 + element_size * 16)
				label:SetTall(p:GetTall())
		...
		-- SF Misc
			...
			t:SetSize(340,20)
			t:SetPos(0,10 + element_size * 18)
			--sf_allow_rainsound
			local p,label = clientToggle(panel,"sf_allow_rainsound")
				p:SetPos(20,10 + element_size * 19)
				label:SetPos(30 + p:GetWide(),10 + element_size * 19)
				label:SetTall(p:GetTall())
			--sf_allow_windsound
