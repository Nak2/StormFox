...
-- Weather Brain
local function LeapVarable(at_zero,at_one,percent) -- Number, table and color
	...
	elseif type(at_zero) == "table" then
		if at_zero.a and at_zero.r and at_zero.g and at_zero.b then
			-- color
			local r = LeapVarable(at_zero.r,at_one.r,percent)
			local g = LeapVarable(at_zero.g,at_one.g,percent)
			local b = LeapVarable(at_zero.b,at_one.b,percent)
			local a = LeapVarable(at_zero.a,at_one.a,percent)
			return Color(r,g,b,a)
		else
			-- A table of stuff? .. or what.
			local tab = table.Copy(at_zero)
			for key,var in pairs(at_one) do
				tab[key] = LeapVarable(at_zero[key],var,percent)
			end
			return tab
		end
...
function StormFox.SetWeather(name,percent)
	...
	StormFox.SetData("Weather",percent > 0 and name or "Clear")
	...
	for key,data in pairs(Weather[name]) do
		if not CurrentWeatherData[key] then
			...
			CurrentWeatherData[key] = LeapVarable(CurrentWeatherData[key],data,percent)
		end
	end
	return CurrentWeatherData
end
